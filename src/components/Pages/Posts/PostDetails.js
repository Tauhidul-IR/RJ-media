...
const PostDetails = () => {
    ...
    const { data: postinfo = [], refetch, isLoading } = useQuery({
        queryKey: [post._id],
        queryFn: async () => {
            const res = await fetch(`https://social-media-server-tauhidul-ir.vercel.app/posts/${post._id}`);
    ...
    const handleAddComment = (event) => {
        ...
        console.log(newPost);
        fetch(`https://social-media-server-tauhidul-ir.vercel.app/posts/${postinfo._id}`, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
